...
import { CreateCourseRequest, SetCourseFileRequest, UpdateCourseRequest} from '../../../globals/interfaces/course';
...
describe('CoursesService', () => {
  ...
  describe('"Create course" method', () => {
    ...
    it('should return a new course object', async () => {
        ...
        expect(createdCourse).toMatchObject({...mockTestingCourse, courseName:newCourseName});
    });

    it('should throw rpc Exception', () => {
      expect(coursesService.create({ ...createCourseRequest, courseName: mockTestingCourse.courseName})).rejects.toThrow(RpcException)
    })
  ...
  describe('Add files to course', () => {
    ...
    it('should connect files to the course', async () => {
        ...
        const response = await coursesService.addFilesToCourse(setCourseFileRequest);
